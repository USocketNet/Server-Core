...
instance.sio.on('connection', (socket) => {
  ...
  usn.debug.log('MASTER SERVER', 'User #' + socket.wpid + ' connect @ port ' + conn.address().port + ' with sid of ' + socket.id, 'white', 'connect');
  ...
  socket.on('disconnect', (reason) => {
    ...
    */
    ...
    usn.debug.log('MASTER SERVER', 'User #' + socket.wpid + ' disconnect @ port ' + conn.address().port + ' with sid of ' + socket.id + ' - ' + reason, 'white', 'disconnect');
  ...
  socket.on('error', (error) => {
    usn.debug.log('MASTER SERVER', 'User #' + socket.wpid + ' disconnect @ port ' + conn.address().port + ' with sid of ' + socket.id + ' - ' + error, 'white', 'disconnect');
  });
});