...
class DocuPress_Ajax {
    ...
    public function __construct() {
        add_action( 'wp_ajax_docupress_create_doc', array( $this, 'create_doc' ) );
        add_action( 'wp_ajax_docupress_clone_doc', array( $this, 'clone_doc' ) );
        add_action( 'wp_ajax_docupress_remove_doc', array( $this, 'remove_doc' ) );
        add_action( 'wp_ajax_docupress_export_doc', array( $this, 'export_doc' ) );
        add_action( 'wp_ajax_docupress_admin_get_docs', array( $this, 'get_docs' ) );
        add_action( 'wp_ajax_docupress_sortable_docs', array( $this, 'sort_docs' ) );

        // feedback.
        add_action( 'wp_ajax_docupress_ajax_feedback', array( $this, 'handle_feedback' ) );
        add_action( 'wp_ajax_nopriv_docupress_ajax_feedback', array( $this, 'handle_feedback' ) );

        // feedback suggestion.
        add_action( 'wp_ajax_docupress_ajax_feedback_suggestion', array( $this, 'handle_feedback_suggestion' ) );
        add_action( 'wp_ajax_nopriv_docupress_ajax_feedback_suggestion', array( $this, 'handle_feedback_suggestion' ) );
    }
    ...
    public function get_post_data( $post ) {
        ...
        return array(
            ...
            'thumb'    => get_the_post_thumbnail_url( $post, 'docupress_archive_sm' ),
    ...
    public function create_doc() {
        check_ajax_referer( 'docupress-admin-nonce' );
    ...
    public function remove_doc() {
        ...
        if ( ! current_user_can( 'delete_post', $post_id ) ) {
            wp_send_json_error( __( 'You are not allowed to delete this item.', 'docupress' ) );
        }
    ...
    public function handle_feedback() {
        check_ajax_referer( 'docupress-ajax' );

        $previous = array();

        if ( isset( $_COOKIE['docupress_response'] ) ) {
            $cookies_data = explode( ',', sanitize_text_field( wp_unslash( $_COOKIE['docupress_response'] ) ) );
        ...
        if (
            ...
            $message = __( 'Sorry, you\'ve already recorded your feedback!', 'docupress' );
        ...
        if ( $type ) {
            ...
            setcookie( 'docupress_response', $cookie_val, time() + WEEK_IN_SECONDS, COOKIEPATH, COOKIE_DOMAIN );
        }

        $message = __( 'Thank you for feedback!', 'docupress' );
    ...
    public function handle_feedback_suggestion() {
        ...
        $email = docupress()->get_option( 'show_feedback_suggestion_email', 'docupress_single', '' );
        ...
        if ( ! $email ) {
            $response = __( 'Sorry, something went wrong on the server side!', 'docupress' );
        ...
        if ( $post && $feedback_type && $suggestion ) {
            $is_sent = DocuPress_Suggestion::send(
                array(
                    'post'          => $post,
                    'from'          => $from,
            ...
            if ( ! $is_sent ) {
                $response = __( 'Sorry, something went wrong with the mail server, your suggestions were not sent!', 'docupress' );
                wp_send_json_error( $response );
                return;
            }
        }

        $response = __( 'Thank you for suggestions!', 'docupress' );
