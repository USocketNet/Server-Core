...
/<meta.*(name=(\"|\')generator(\"|\'))[^>]*?>/i';
     ...
     */
    public $custom_css =
    '.docupress-single-feedback,
.docupress-search-form,
.docupress-search-form-result {
    display: none;
}
body {
    ...
    public function __construct() {
        ...
        $export_folder     = 'docupress-export';
    ...
    public function run_single( $permalink, $file_name, $export_path ) {
        ...
        $custom_js = docupress()->get_option( 'custom_js', 'docupress_export', $this->custom_js );
        if ( $custom_js ) {
            $content = str_replace( '</head>', "\n<script>" . $custom_js . "</script>\n\n</head>", $content );
        }
        $custom_css = docupress()->get_option( 'custom_css', 'docupress_export', $this->custom_css );
        if ( $custom_css ) {
...
LOD;
    ...
    public function clean_html( $html ) {
        $regexp_array = explode( "\n", docupress()->get_option( 'clean_html', 'docupress_export', $this->clean_html_regexp ) );
